...
> [!NOTE]
> **当前最新稳定版为 `v3.1.2`。** `v2.5.17` 已因重大升级缺陷撤回，内部候选 `v3.0.6` 从未发布；现有用户可以直接升级到 `v3.1.2`。CS2、内嵌英雄联盟工作台、无畏契约工具与外设调优均包含在同一个安装包中。
...
<p align="center">
  ...
  <a href="./docs/releases/v3.1.2.zh-CN.md">v3.1.2 更新说明</a> •
...
- **统一存储选择** — 设置可显示应用自有数据、后端日志、缓存、WebView 数据和英雄联盟临时文件的当前、待切换与保留原目录。
- **校验后切换** — 存储切换会在下一次启动时复制并校验，执行前可以取消，并保留原目录用于恢复。
- **安装器与管理员修复** — 新安装优先选择可用的非系统固定磁盘但不假定 `D:`；尊重显式 `/D=` 和已有升级路径；存储不可用时不静默回退到 `C:`，UAC 前的一次性启动脚本改用已选的可写临时目录。

完整更新说明见 [`docs/releases/v3.1.2.zh-CN.md`](./docs/releases/v3.1.2.zh-CN.md)。
...
> **当前稳定版本：`v3.1.2`。** 已签名安装包与更新清单通过仓库的稳定版 Release 工作流发布。
...
| **v3.1.2 · 安装器与统一存储** | 修复管理员 UAC 前后临时路径边界；新增保留原目录的校验后待切换存储；尊重显式安装和升级位置，不静默回退到系统盘 | ✅ 当前稳定版 |
