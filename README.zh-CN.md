...
> [!NOTE]
> **当前最新稳定版为 `v3.1.4`。** `v2.5.17` 已因重大升级缺陷撤回，内部候选 `v3.0.6` 从未发布；现有用户可以直接升级到 `v3.1.4`。CS2、内嵌英雄联盟工作台、无畏契约工具与外设调优均包含在同一个安装包中。
...
<p align="center">
  ...
  <a href="./docs/releases/v3.1.4.zh-CN.md">v3.1.4 更新说明</a> •
...
- **后端按需启动** — 首页、检查更新和英雄联盟入口不再等待 Python，依赖后端的页面首次使用时才启动服务。
- **RESG 可记忆置顶** — 可在数据窗口或窗口管理中开启、关闭置顶，并自动保存选择。
- **改善 GitHub 更新体验** — 检查超时与取消安装提示更明确，不增加付费加速服务。

完整更新说明见 [3.1.4 发布说明](./docs/releases/v3.1.4.zh-CN.md)。

## v3.1.0 上一稳定版

上一版本引入了单一内嵌英雄联盟工作台、可逆启动方式、可选仅工作台管理员启动，以及受保护的运行时打包边界。完整说明见 [`docs/releases/v3.1.0.zh-CN.md`](./docs/releases/v3.1.0.zh-CN.md)。

---

## v3.0.6 历史本地候选（未发布，已由 v3.1.1 取代）

> [!WARNING]
> `v3.0.6` 从未发布，也没有对应的 GitHub Release 或正式更新通道。以下内容保留作历史候选记录，它已由稳定版 `v3.1.0`、后续的 `v3.1.1` 取代；客户端更新弹窗不会把它当作可升级版本。
...
> **当前稳定版本：`v3.1.4`。** 已签名安装包与更新清单通过仓库的稳定版 Release 工作流发布。
...
| **v3.1.0 · 内嵌英雄联盟工作台** | 固定 v1.5.1 源码构建并内嵌单一工作台；可选节省内存或后台并行，支持可逆记忆和受保护的工作台单独管理员启动 | ✅ 上一稳定版 |
| **v3.1.1 · 内嵌工作台关闭流程** | 为工作台 `X` 按钮增加返回或最小化的明确选择，支持安全取消，并统一两种启动模式的关闭行为 | ✅ 上一稳定版 |
| **v3.1.2 · 安装器与统一存储** | 修复管理员 UAC 前临时路径边界；显示存储位置并支持手动切换，不自动迁移数据 | ✅ 上一稳定版 |
| **v3.1.4 · 按需后端与 RESG 置顶** | 独立启动、可记忆置顶与更明确的 GitHub 更新提示 | ✅ 当前稳定版 |
| **v3.1.3 · RESG 与托盘** | 海克斯大乱斗独立数据窗口、可逆精选视图与真正的系统托盘隐藏 | 已发布 |

### 待做

- [ ] **跨设备 CFG 同步** — 使用 Google Drive 或 WebDAV 存储并同步 CS2 与 VALORANT 的 CFG，减少同一设备切换不同账号、或同一账号跨设备使用时手动调整配置的麻烦。
