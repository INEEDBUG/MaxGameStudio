...
> [!NOTE]
> **当前最新稳定版为 `v3.1.1`。** `v2.5.17` 已因重大升级缺陷撤回，内部候选 `v3.0.6` 从未发布；现有用户可以直接升级到 `v3.1.1`。CS2、内嵌英雄联盟工作台、无畏契约工具与外设调优均包含在同一个安装包中。
...
<p align="center">
  ...
  <a href="./docs/releases/v3.1.1.zh-CN.md">v3.1.1 更新说明</a> •
...
- **明确关闭选择** — 点击内嵌英雄联盟工作台右上角 `X` 后，会询问返回 MaxGameStudio，或最小化英雄联盟工作台并保留运行状态。
- **安全取消** — 点击取消会保留当前工作台和宿主状态，不会关闭窗口、停止后台任务或结束游戏进程。
- **两种模式一致** — “节省内存”和“后台并行”使用一致的关闭确认规则，减少误退出，同时保留各自的内存行为。

完整更新说明见 [`docs/releases/v3.1.1.zh-CN.md`](./docs/releases/v3.1.1.zh-CN.md)。
...
> **当前稳定版本：`v3.1.1`。** 已签名安装包与更新清单通过仓库的稳定版 Release 工作流发布。
...
| **v3.1.1 · 内嵌工作台关闭流程** | 为工作台 `X` 按钮增加返回或最小化的明确选择，支持安全取消，并统一两种启动模式的关闭行为 | ✅ 当前稳定版 |
