...
> **同一个桌面应用：** CS2 工作台、League 实验室与 VALORANT 实验室都内置在 MaxGameStudio 中，从同一侧栏进入，不需要单独安装。
...
- **英雄联盟实验室**：通过本机 LCU/SGP 连接 Riot 与 WeGame/Tencent 客户端，提供带明确开关与确认的对局流程辅助、英雄/符文/召唤师技能配置、玩家与战绩分析，以及可选的 Mini、实时对局和技能冷却辅助窗口。账号写入功能默认受保护；客户端凭据只在运行内存中使用，不进入导出文件或上传。
- **无畏契约实验室**：读取本机 GPU、主监视器与刷新率，检测完成后可预览并在确认后应用 `1568×1080` 或自定义显示模式，倒计时内可保留或恢复；软件不会自动禁用监视器。准心编辑器支持 P/A/S 配置、实时预览、导入导出与本机回退，回退保存不保证与原生代码等价。
- 英雄联盟集成参考了 [LeagueAkari](https://github.com/LeagueAkari/LeagueAkari)（MIT）的 LCU 与游戏流程思路；本项目独立实现并随仓库保留 MIT 许可证与致谢：`third_party/licenses/LeagueAkari-LICENSE.txt`。
...
前往本项目的 [Releases 页面](https://github.com/INEEDBUG/MaxGameStudio/releases)，找到最新版本并下载 **`MaxGameStudio_x.x.x_x64-setup.exe`**。
...
- 如果有新版本，界面会显示版本号、更新说明和下载进度
- 程序会自动下载并校验签名，不需要再手动打开 GitHub 或运行安装包
- 下载完成后程序会关闭旧版本、覆盖安装并自动启动新版本
