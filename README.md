...
> [!NOTE]
> **当前稳定版为 `v3.0.3`。** `v2.5.17` 已因重大升级缺陷撤回；用户应直接升级到 `v3.0.3`。CS2 工作台、英雄联盟与无畏契约功能已经合并在同一个安装包中。
...
<p align="center">
  ...
  <a href="#v303-更新重点">v3.0.3 更新重点</a> •
  <a href="#核心功能">核心功能</a> •
  <a href="#安装">快速安装</a> •
...
> [!TIP]
> 英雄联盟是本版本的重点优化对象。完整技术说明与安装包见 [v3.0.3 Release](https://github.com/INEEDBUG/MaxGameStudio/releases/tag/v3.0.3)；客户端更新弹窗会改用面向普通用户的“修复 / 新功能 / 优化”摘要。
...
- **功能更容易找到** — 已稳定的战绩、玩家中心和实时对局提升为正式入口；旧链接继续兼容并保留查询参数。
- **页面状态更可靠** — 修复页面切换后的旧请求覆盖新状态、卸载后继续更新、空数据与失败状态不清楚等问题。
- **Mini 生命周期更稳定** — 原生窗口同步保留最新待处理状态，快速进入游戏阶段时不会因为丢失隐藏事件而继续覆盖游戏画面。
- **减少重复读取** — 四个常驻管理器共享同一份 1.5 秒状态源；固定一分钟模型下请求由 60 次降至 40 次，实时对局嵌入页的额外状态请求由每分钟 12 次降为 0。
- **降低重复 IO** — 十名玩家分析时，本地标签文件由最多读取 10 次降为每轮 1 次；并发状态读取在 350ms 窗口内合并为一次实际刷新。

上述数字来自固定代码路径和自动化测试模型，不代表所有电脑上的绝对延迟或内存。发布验收没有自动执行英雄联盟账号动作，也没有伪造完整 Lobby → ReadyCheck → ChampSelect → InProgress 实机链路。
...
以下截图来自 `v2.5.16` 桌面端 UI 基线和本地演示环境，不应视为 `v3.0.3` 的专属截图。演示账号、断开连接与空数据状态仅用于展示界面，不包含真实 SteamID、Riot ID、比赛记录或本机路径。点击图片可查看原图。
...
从 `v2.5.9` 起，客户端通过签名更新通道检查新版本；从 `v2.5.11` 起，正式版会由 GitHub Actions 构建并发布。客户端启动时会检查一次，运行或驻留后台期间每 15 分钟继续检查；发现**正式版本**后会按“修复 / 新功能 / 优化”展示普通用户可以直接理解的更新说明，并自动完成下载、签名校验、覆盖安装和新版重启。普通更新可以选择暂不升级当前版本，手动检查更新仍可重新打开提示。配置、Demo 数据库和工作区数据保存在独立的用户数据目录中，覆盖安装不会删除它们。Release 页面仍只保留一个面向普通用户的 Windows EXE。
...
| **v3.0.3 · 游戏分区与 League 稳定性** | 按游戏重组导航；英雄联盟自动化、战绩、玩家中心、实时对局和工具箱改为直达入口；合并重复状态请求并修复 Mini、页面切换与异步竞态 | ✅ 当前稳定版 |

### 待做

- [ ] **高级战术教练** — 在现有投掷物统计、轨迹与路线复盘之上，补充路线对比、投掷物质量评分和可解释的自动战术建议。
- [ ] **复盘展示增强** — 为基础战术指标增加更清晰的分局对比、筛选和可导出报告。
- [ ] **发布可信度** — 为 Windows 安装包补齐公开可验证的 Authenticode 代码签名；现有 Tauri 更新包签名与安装包系统信任是两条不同的验证链。
