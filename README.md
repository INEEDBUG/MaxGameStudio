...
> [!NOTE]
> **当前稳定版为 `v2.5.16`。** CS2 工作台、英雄联盟实验室和无畏契约实验室已经合并在同一个安装包中；正式客户端可通过签名更新通道完成覆盖安装。
...
以下截图来自当前 `v2.5.16` 桌面端和本地演示环境。演示账号、断开连接与空数据状态仅用于展示界面，不包含真实 SteamID、Riot ID、比赛记录或本机路径。点击图片可查看原图。
