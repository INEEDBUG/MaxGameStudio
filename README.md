...
<h4 align="center">CS2 Demo 分析与录制 · 英雄联盟实验室 · 无畏契约实验室</h4>

> **一个 MaxGameStudio 桌面应用：** CS2 工作台、League 实验室与 VALORANT 实验室共用同一个 Tauri 客户端和侧栏入口；两个实验室不是独立安装的桌面程序。

> [!NOTE]
> **当前稳定版为 `v3.0.1`。** `v2.5.17` 已因重大升级缺陷撤回；已安装该版本的用户应升级到 `v3.0.1`，尚未升级的用户请继续使用 `v2.5.16`。CS2 工作台、英雄联盟实验室和无畏契约实验室已经合并在同一个安装包中。
...
以下截图来自 `v2.5.16` 桌面端 UI 基线和本地演示环境，不应视为 `v3.0.1` 的专属截图。演示账号、断开连接与空数据状态仅用于展示界面，不包含真实 SteamID、Riot ID、比赛记录或本机路径。点击图片可查看原图。
...
| **v3.0.1 · 升级修复** | 修复安全清理、版本跳过/强制更新和 Release 标题版本识别问题 | ✅ 当前稳定版 |
