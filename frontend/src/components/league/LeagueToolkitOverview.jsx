...
import { AlertTriangle, AppWindow, Boxes, Gift, Maximize2, MessageSquare, PlayCircle, Power, RefreshCw, Settings2, Trophy, Users } from "lucide-react";
...
const LeagueInGamePresetToolsImpl = lazy(() => import("./LeagueInGamePresetTools"));
...
export default function LeagueToolkitOverview({ onError, settings, onSettingsUpdate, onDryRunGame, onOpenPlayer, requestedTab = "", streamerMode = Boolean(settings?.streamer_mode_enabled), useAliases = Boolean(settings?.streamer_mode_use_aliases) }) {
  ...
  const normalizedRequestedTab = TOOLKIT_TABS.some(([id]) => id === requestedTab) ? requestedTab : "";
  ...
  return <div className={`space-y-4 toolkit-overview-${toolkitTab}`}>
    ...
    {(toolkitTab === "lobby" || toolkitTab === "misc") && <LazyToolkitPanel><LeagueAdvancedToolkitImpl section={toolkitTab} enabled={Boolean(settings?.toolkit_account_actions_enabled)} busy={busy} onBusyChange={setBusy} onError={onError} onDryRunGame={onDryRunGame} onOpenPlayer={onOpenPlayer} streamerMode={streamerMode} useAliases={useAliases}/></LazyToolkitPanel>}
    {toolkitTab === "in-game-send" && <LazyToolkitPanel><LeagueInGamePresetToolsImpl settings={settings} busy={busy} onSettingsUpdate={onSettingsUpdate} onBusyChange={setBusy} onError={onError} streamerMode={streamerMode} useAliases={useAliases}/></LazyToolkitPanel>}
    {toolkitTab === "in-process" && <InProcessTools status={clientStatus} enabled={Boolean(settings?.toolkit_account_actions_enabled)} busy={busy} onBusyChange={setBusy} onError={onError} onRefresh={load}/>}
    {toolkitTab === "misc" && <>
