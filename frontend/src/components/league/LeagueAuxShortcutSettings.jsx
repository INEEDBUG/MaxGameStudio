...
export default function LeagueAuxShortcutSettings({ settings, busy, onSettingsUpdate }) {
  ...
  return <section className="rounded-2xl border border-cyan-400/20 bg-cs2-bg-elevated p-4">
    <div className="flex items-start gap-3"><Keyboard className="mt-0.5 h-4 w-4 text-cyan-300"/><div><h3 className="text-sm font-bold">League 全局窗口与取消快捷键</h3><p className="mt-1 text-xs text-cs2-text-muted">留空即关闭。实时对局窗口仅在按住快捷键时显示；其余窗口按一次切换显示状态。快捷键不会绕过任何账号写入保护。</p></div></div>
