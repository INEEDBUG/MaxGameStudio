...
import { Crown, Palette, Search } from "lucide-react";
import {
...
export default function LeagueAdvancedToolkit({ enabled, busy, onBusyChange, onError, onDryRunGame = () => {}, onOpenPlayer = () => {}, streamerMode = false, useAliases = false }) {
  ...
  const load=async()=>{const request=++loadRequest.current;onBusyChange(true);try{const catalog=await fetchLeagueChampions();if(disposed.current||request!==loadRequest.current)return;setChampions(Array.isArray(catalog?.champions)?catalog.champions:[]);}catch(error){if(!disposed.current&&request===loadRequest.current)onError(error?.response?.data?.detail||"高级工具读取失败");}finally{if(!disposed.current&&request===loadRequest.current)onBusyChange(false);}};
